...
namespace {
	/**
	 * Builds a timed normal line with words and annotations.
	 */
	Line makeNormalLine() {
		WordNormal w1;
...
TEST_CASE("asLineNormal and asLineInterlude unwrap the matching variant") {
	const Line normal    = makeNormalLine();
	const Line interlude = makeInterludeLine();
	REQUIRE(asLineNormal(normal) != nullptr);
	CHECK(asLineNormal(normal)->content().words_size() == 3);
	CHECK(asLineNormal(interlude) == nullptr);
	CHECK(asLineInterlude(interlude) != nullptr);
	CHECK(asLineInterlude(normal) == nullptr);
}
